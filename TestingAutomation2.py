...
def Create_Testing_Report():
    Testing_File = filedialog.askopenfilename(title="Select the Testing File")
    Status_Report_File = filedialog.askopenfilename(title= "Select the Status Report")
    Composition_Report_File = filedialog.askopenfilename(title= "Select the Composition Report")
    Remediation_Report_File = filedialog.askopenfilename(title= "Select the Remediation Report")
    SignOff_Report_File = filedialog.askopenfilename(title= "Select the Sign Off Report")
    Post_Report_File = filedialog.askopenfilename(title= "Select the POST Report")
    App_Name = input('Enter the App Name: ')

    try: 
       Testing_File = pd.read_excel(Testing_File)
       Attestation_File = pd.DataFrame()
       ...
       with pd.ExcelWriter(Output_filename, engine='xlsxwriter') as writer:
           Testing_File.to_excel(writer, sheet_name="Testing", index=False, header=False)
           Attestation_File.to_excel(writer, sheet_name="Attestation", index=False)
